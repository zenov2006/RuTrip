...
async function saveRegionData() {
    ...
    if (!regionId) { showToast('Ошибка: регион не найден', 'error'); return; }
    const reviewData = {
...
function setupMapInteractivity() {
    ...
    paths.forEach(path => {
        ...
        path.addEventListener('click', () => {
            ...
            if (regionId && visitedRegions[regionId]) showRegionInfo(title, visitedRegions[regionId]);
            else startRegionAdd(title);
        });
    });
}

function showRegionInfo(regionName, data) {
    const modalHtml = `<div class="modal-overlay" id="regionInfoModal"><div class="modal-container"><button class="modal-close" onclick="closeModal()"><i class="fas fa-times"></i></button><div class="modal-icon"><i class="fas fa-map-marker-alt"></i></div><h2>${regionName}</h2><div style="background:rgba(76,175,80,0.1);border-radius:16px;padding:16px;margin:16px 0;"><p><i class="fas fa-calendar-check"></i> Посещён: ${new Date(data.visitedAt).toLocaleDateString('ru-RU')}</p><p><i class="fas fa-star"></i> Оценка: ${'★'.repeat(data.rating || 5)}${'☆'.repeat(5 - (data.rating || 5))}</p>${data.reviewText ? `<p><i class="fas fa-comment"></i> "${data.reviewText}"</p>` : ''}</div><div class="modal-buttons"><button class="modal-btn secondary" onclick="showDeleteConfirm('${regionName}', '${regionsData[regionName]?.id}')"><i class="fas fa-trash"></i> Удалить</button><button class="modal-btn primary" onclick="closeModal()">Закрыть</button></div></div></div>`;
    closeModal();
    document.body.insertAdjacentHTML('beforeend', modalHtml);
