...
// ==================== УПРАВЛЕНИЕ ДРУЗЬЯМИ (НОВАЯ ВЕРСИЯ) ====================
...
function openFriendsModal() {
    ...
    const modalHtml = `
        <div class="modal-overlay" id="friendsModal">
            <div class="modal-container" style="max-width: 500px;">
                ...
                <h2 style="text-align:center;margin-bottom:24px;"><i class="fas fa-user-friends"></i> Управление друзьями</h2>
                
                <!-- Блок добавления друга -->
                <div style="margin-bottom: 30px; background: var(--secondary); padding: 20px; border-radius: 16px;">
                    <h3 style="margin-bottom: 15px; font-size: 18px; display: flex; align-items: center; gap: 8px;">
                        <i class="fas fa-user-plus" style="color: var(--accent);"></i> Добавить друга
                    </h3>
                    <div class="form-group">
                        <label>ID друга</label>
                        <input type="text" id="friendIdInput" class="form-input" placeholder="Введите ID пользователя..." style="margin-bottom: 10px;">
                        <p style="font-size: 12px; color: #999;">ID можно найти в профиле пользователя</p>
                    </div>
                    <button class="modal-btn primary" style="width:100%;" onclick="addFriendById()">
                ...
                <div style="display:flex; gap:10px; margin-bottom:20px; flex-wrap:wrap;">
                    <button id="myFriendsTab" class="modal-btn primary" style="flex:1;">👥 Мои друзья</button>
                    <button id="requestsFriendsTab" class="modal-btn secondary" style="flex:1;">📨 Заявки</button>
                </div>
                
                <div id="friendsModalContent">
                    <div id="myFriendsPanel">
                        <div id="friendsListModal" style="max-height: 350px; overflow-y: auto;">
                            ${renderFriendsListModal()}
                        </div>
                    </div>
                    
                    <div id="requestsFriendsPanel" style="display:none;">
                        <h4 style="margin-bottom: 10px;">Входящие заявки</h4>
                        <div id="incomingRequestsList" style="max-height: 150px; overflow-y: auto;"></div>
                        <h4 style="margin-top: 20px; margin-bottom: 10px;">Исходящие заявки</h4>
                        <div id="outgoingRequestsList" style="max-height: 150px; overflow-y: auto;"></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    document.getElementById('myFriendsTab').onclick = () => {
        document.getElementById('myFriendsPanel').style.display = 'block';
        document.getElementById('requestsFriendsPanel').style.display = 'none';
        document.getElementById('myFriendsTab').className = 'modal-btn primary';
        document.getElementById('requestsFriendsTab').className = 'modal-btn secondary';
        // Обновляем список друзей при переключении
        const friendsListDiv = document.getElementById('friendsListModal');
        if (friendsListDiv) {
            friendsListDiv.innerHTML = renderFriendsListModal();
        }
    };
    
    document.getElementById('requestsFriendsTab').onclick = () => {
        document.getElementById('myFriendsPanel').style.display = 'none';
        document.getElementById('requestsFriendsPanel').style.display = 'block';
        document.getElementById('myFriendsTab').className = 'modal-btn secondary';
...
function renderFriendsListModal() {
    if (!friendsList || friendsList.length === 0) {
        return `<div style="text-align:center;padding:40px;color:#999;">
            <i class="fas fa-users" style="font-size:48px;margin-bottom:16px;display:block;"></i>
            <p>У вас пока нет друзей</p>
            <p style="font-size:14px;">Добавьте друга по ID выше!</p>
        </div>`;
    }
    ...
    return friendsList.map(friend => `
        <div style="display:flex; justify-content:space-between; align-items:center; padding:12px; border-bottom:1px solid #eee;">
            <div>
                <strong style="font-size:16px;">${friend.name}</strong>
                <div style="font-size:12px; color:#999; margin-top:4px;">
                    <i class="fas fa-id-card"></i> ID: ${friend.id}
                </div>
            </div>
            <button class="modal-btn secondary" style="padding:6px 16px;" onclick="viewFriendMap('${friend.id}')">
...
async function addFriendById() {
    ...
    if (!friendId) {
        showToast('Введите ID друга', 'info');
        return;
    }
    ...
    if (friendsList.some(f => f.id == friendId)) {
        showToast('Этот пользователь уже ваш друг', 'info');
        return;
    }
    
    // Проверяем, нет ли уже отправленной заявки
    if (friendRequests.outgoing?.some(r => r.toUserId == friendId)) {
        showToast('Заявка этому пользователю уже отправлена', 'info');
        return;
    }
    
    try {
        // Сначала проверяем существование пользователя
        const checkResponse = await fetch(`${API_BASE}/users/${friendId}`, {
            headers: getAuthHeaders()
        });
        
        if (!checkResponse.ok) {
            showToast('❌ Пользователь с таким ID не найден', 'error');
            return;
        }
        ...
        if (success) {
            ...
            // Если вкладка заявок открыта, обновляем её
            if (document.getElementById('requestsFriendsPanel')?.style.display === 'block') {
                await loadRequestsList();
            }
        }
    } catch (e) {
        console.error('Ошибка:', e);
        showToast('Ошибка при добавлении друга', 'error');
    }
}
        ...
        incomingDiv.innerHTML = friendRequests.incoming.map(req => `
            <div style="display:flex; justify-content:space-between; align-items:center; padding:12px; border-bottom:1px solid #eee;">
                <div>
                    ...
                    <div style="font-size:11px; color:#999; margin-top:3px;">ID: ${req.fromUser?.id || '?'}</div>
                </div>
                <div>
                    <button class="modal-btn primary" style="padding:4px 12px; margin-right:8px;" onclick="acceptFriendRequestFromModal('${req.id}')">Принять</button>
        ...
        outgoingDiv.innerHTML = friendRequests.outgoing.map(req => `
            <div style="display:flex; justify-content:space-between; align-items:center; padding:12px; border-bottom:1px solid #eee;">
                <div>
                    ...
                    <div style="font-size:11px; color:#999; margin-top:3px;">ID: ${req.toUser?.id || '?'}</div>
                </div>
                <div>
                    <span style="color:#ff9800; font-size:13px;"><i class="fas fa-clock"></i> ожидание</span>
                </div>
            </div>
        `).join('');
    } else {
...
async function acceptFriendRequestFromModal(requestId) {
    ...
    await fetchFriends();
    updateFriends();
    // Обновляем список друзей в модальном окне
    const friendsListDiv = document.getElementById('friendsListModal');
    if (friendsListDiv) {
        friendsListDiv.innerHTML = renderFriendsListModal();
    }
...
// =============================================
// Убедитесь, что эти функции существуют или добавьте их:
...
async function fetchFriends() {
    ...
    try {
        ...
        console.log('Ошибка загрузки друзей:', e);
    }
    return [];
}
...
async function fetchFriendRequests() {
    ...
    try {
        const response = await fetch(`${API_BASE}/users/me/friend-requests`, { 
            headers: getAuthHeaders() 
        });
        if (response.ok) {
            const data = await response.json();
        ...
        console.log('Ошибка загрузки заявок:', e);
    }
    return { incoming: [], outgoing: [] };
}
